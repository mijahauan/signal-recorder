#!/usr/bin/env python3
"""
Generate daily spectrograms from 10 Hz carrier channel archives

Reads 10 Hz IQ data directly from radiod carrier channels (CW mode, 10 Hz bandwidth).
This eliminates decimation artifacts since radiod provides properly filtered 10 Hz data.

Advantages over decimation approach:
- No aliasing artifacts (radiod's filtering is superior)
- No decimation errors on short files
- Cleaner spectrograms for carrier Doppler analysis
- Smaller file size (10 Hz vs 16 kHz)
- Perfect for carrier visualization (±5 Hz range)

Usage:
    python3 scripts/generate_spectrograms_from_carrier.py --date 20251116
"""
...
def find_carrier_npz_files(carrier_archive_dir: Path, date_str: str) -> List[Path]:
    """
    Find all carrier channel NPZ files for a specific date
    
    Args:
        carrier_archive_dir: Carrier channel archive directory
        date_str: Date in YYYYMMDD format
        
    Returns:
        Sorted list of carrier NPZ file paths
    """
    if not carrier_archive_dir.exists():
        logger.warning(f"Carrier archive directory not found: {carrier_archive_dir}")
        return []
    
    # Carrier NPZ files: YYYYMMDDTHHMMSSZ_freq_iq.npz
    npz_files = sorted(carrier_archive_dir.glob(f"{date_str}*_iq.npz"))
    
    logger.info(f"Found {len(npz_files)} carrier NPZ files for {date_str}")
    return npz_files
...
def main():
    ...
    for channel_name in channels:
        ...
        try:
            ...
            if not npz_files:
                logger.warning(f"No carrier NPZ files found for {channel_name}")
                continue
